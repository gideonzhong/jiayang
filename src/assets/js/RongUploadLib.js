...
(function (RongIMLib) {
    var RongUploadLib = (function () {
        function RongUploadLib(imgOpts, fileOpts) {
            ...
            plScript.onload = plScript.onreadystatechange = function () {
                if (!this.readyState || this.readyState == 'loaded' || this.readyState == 'complete') {
                    ...
                    qiniuScript.src = "http://cdn.ronghub.com/qiniu.min.js";//融七
        ...
        RongUploadLib.prototype.createOptions = function (opts, type) {
            // console.log("createOptions"+opts);
            var me = this;
            if (!opts)
                return;
        ...
        RongUploadLib.prototype.createMessage = function (option, file, callback) {
            ...
            switch (option.uploadType) {
                case 'IMAGE':
                    ...
                    var user = {
                        id: webimutil.CookieHelper.getCookie("loginuserid"),
                        nickName: webimutil.CookieHelper.getCookie("userselfname"),
                        portraitUri: webimutil.CookieHelper.getCookie("userselfavater")
                    }
                    ...
                    //         // 以后的图片消息   都是按照这个消息类型发送图片
                    //         var msg = new RongIMClient.RegisterMessage.ImgMessage({mMediaUrl: data.downloadUrl,user:user});
                    //         console.log("本地路径？？？？？？" + data.downloadUrl +user);
                    //         // console.log("88888"+msg);
                    //         callback(msg);
                    //         console.log(msg)
                    //         });
                    ...
                    RongIMLib.RongIMClient.getInstance().getFileUrl(RongIMLib.FileType.IMAGE, option.fileName, null, {
                        onSuccess: function (data,user) {
                        	//console.log(data);//一个地址
                            if (option.isBase64Data) {
                               
                                //这个是融云的发送图片类型的消息    ImageMessage
...
//                              });
								//RongUploadLib.imageCompressToBase64(file.getNative(), function (content) {
                                    //这个是第一次我们定义的自定义图片消息     可以删除了
                                    // msg = new  RongIMClient.RegisterMessage.ImageViewMessage({ imageViewUrl: data.downloadUrl });
                                    msg = new  RongIMClient.RegisterMessage.ImgMessage({ mMediaUrl: data.downloadUrl,user:user});
                                    // console.log("0001");
                                    console.log(msg)
                                    callback(msg);
                                //});
...
//                              });
                                //RongUploadLib.imageCompressToBase64(file.getNative(), function (content) {
                                    //以后的图片消息   都是按照这个消息类型发送图片
                                    msg = new  RongIMClient.RegisterMessage.ImgMessage({ mMediaUrl: data.downloadUrl,user:user});
                                    console.log("本地路径？？？？？？"+data.downloadUrl);
                                    console.log("0002");
                ...
                case 'FILE':
                    ...
                    var user = {
                        id: webimutil.CookieHelper.getCookie("loginuserid"),
                        name: webimutil.CookieHelper.getCookie("userselfname"),
                        portrait: webimutil.CookieHelper.getCookie("userselfavater")
                    }
                    ...
                    RongIMLib.RongIMClient.getInstance().getFileUrl(RongIMLib.FileType.FILE, option.fileName, encodeURIComponent(file.oldName), {
                        onSuccess: function (data) {
                            ...
                            msg = new RongIMLib.FileMessage({ name: file.oldName, size: file.size, type: type, fileUrl: data.downloadUrl, user: user });
                            callback(msg);
                        },
                        onError: function (error) { }
                    });
