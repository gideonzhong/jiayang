    ...
    function ($q: any, mainDataServer: mainDataServer, mainServer: mainServer, RongIMSDKServer: RongIMSDKServer) {
        ...
        function messageAddUserInfo(item: webimmodel.Message) {
            ...
            if (mainDataServer.conversation.currentConversation.targetId == item.targetId) {
                if (item.content.content == "管理员设置了全员禁言" && item.senderUserId != loginuserid) {
                    $('#message-content').css('display', 'none');
                    $('#MessageForm-header').css('display', 'none');
                    $('.sendBtn').html("群主已设置全群禁言");
                } else if (item.content.content == "管理员取消了全员禁言") {
                    $('#message-content').css('display', 'block');
                    $('#MessageForm-header').css('display', 'block');
                    $('.sendBtn').html("发送");
                }
            }
            ...
            if (item.messageDirection == 1) {
                ...
                // alert(webimmodel.conversationType.Group);
                switch (item.conversationType) {
                    ...
                    case webimmodel.conversationType.Group:
                        ...
                        //			        		$('.sendBtn').html("发送");
                        //			    		}	
                        //		                	for(var i=0;i<mainDataServer.conversation.conversations.length;i++){
                        //		                		if(mainDataServer.conversation.conversations[i].targetType==3){
                        //		                		}
                        //		                	}
                ...
                if (user) {
                    ...
                    //     console.log("12345")
                    //     if (rep.code == 1) {
                    //         item.senderUserName = rep.content.userInfo.name;
                    //         item.senderUserImgSrc = webimutil.ChineseCharacter.getPortraitChar(rep.content.userInfo.name);
                    //         item.imgSrc = rep.content.userInfo.avatar;
                    //         var _friend = new webimmodel.Friend({
                    //             id: item.senderUserId,
                    //             name: item.senderUserName + '(非好友)',
                    //             imgSrc: item.imgSrc
        ...
        function sendReadReceiptMessage(id: string, type: number, messageuid: string, sendtime: number) {
            ...
            RongIMSDKServer.sendMessage(type, id, msg).then(function () {
                ...
                console.log('sendReadReceiptMessage error00', error.errorCode);
            });
        }
        ...
        function sendSyncReadStatusMessage(id: string, type: number, sendtime: number) {
            ...
            RongIMSDKServer.sendMessage(type, id, msg).then(function () {
                ...
                console.log('sendSyncReadStatusMessage error11', error.errorCode);
            });
        }
