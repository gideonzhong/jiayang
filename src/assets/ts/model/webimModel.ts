...
module webimmodel {
    ...
    export var MessageType = {
        ...
        ImageViewMessage: "ImageViewMessage",
    ...
    export class Message extends ChatPanel {
        ...
        static convertMsg(SDKmsg: any) {
            ...
            switch (msg.messageType) {
                ...
                default:
                    ...
                    }else if(SDKmsg.objectName == "app:ImgMessage"){
				           // console.log(SDKmsg);
				            //console.log(msg);
			                var imageViewUrl = new ImageViewMessage();
			                //console.log(imageViewUrl);
			                
			                //imageViewUrl.imageViewUrl = SDKmsg.content.imageViewUrl;
			
			                msg.content = SDKmsg.content.imageViewUrl;
			            	
                    }
                    
                    else if(SDKmsg.objectName == "app:ImgMedialMsg"){
                        // console.log(SDKmsg);
                         //console.log(msg);
                         var mMediaUrl = new ImgMessage();
                         //console.log(imageViewUrl);
                         
                         //imageViewUrl.imageViewUrl = SDKmsg.content.imageViewUrl;
         
                         msg.content = SDKmsg.content.mMediaUrl;
                         
                 }
              
                    else if(SDKmsg.objectName == "app:VideoMessage"){//视频消息
				            //console.log(SDKmsg);
							var videoMessage = new VideoMessage();
				            videoMessage.content = SDKmsg.content;
                            // console.log(videoMessage);
                            
                            msg.content = SDKmsg.content;//videoPicture图片
			            	
                    }
                   
                    else if(SDKmsg.objectName == "app:VideoMsg"){//视频消息
                        //console.log(SDKmsg);
                        var videoMsg = new VideoMsg();
                        videoMsg.content = SDKmsg.content;
                        // console.log("55555"+videoMsg);
				        msg.content = SDKmsg.content;//videoPicture图片
                        // videoMsg.mMediaUrl = SDKmsg.content.mMediaUrl;
                        // msg.content = SDKmsg.content.mExtra;

                        // console.log("mMediaUrl==="+videoMsg.mMediaUrl+"</br> mExtra==="+msg.content);
                                
                }
               
                    else if(SDKmsg.objectName == "app:RedEnvelopesMessage"){//视频消息
				            msg.content = '红包消息请去手机端查看';
                       		msg.panelType = webimmodel.PanelType.InformationNotification;
                    ...
                    else
                    {
                       msg.content = '当前版本暂不支持查看此消息';
                       msg.panelType = webimmodel.PanelType.InformationNotification;
                       console.log("has unknown message type " + SDKmsg)
                    }
                    break;
            }
        ...
        static messageToNotification = function(msg: any, operatorid: string, isnotification: boolean) {
            ...
            if (msgtype == MessageType.ImageMessage) {
                msgContent = "[图片]";
            } else if (msgtype == MessageType.ImageViewMessage) {
                msgContent = "[图片]";
            }
            
            else if (msgtype == MessageType.ImgMessage) {
                msgContent = "[图片]";
            }
            else if (msgtype == MessageType.VideoMessage) {
                msgContent = "[视频]";
            }
            else if (msgtype == MessageType.VideoMsg) {
                msgContent = "[视频]";
            }
            ...
            // }
            	...
            	//console.log(msgtype+','+MessageType.ImageMessage);
            	if (msg.objectName == "app:ImgMessage") {
	                msgContent = "[图片]";
                }
                if (msg.objectName == "app:ImgMedialMsg") {
	                msgContent = "[图片]";
	            }else if(msg.objectName == "app:RedEnvelopesMessage"){//视频消息
				    msgContent = '[红包消息请去手机端查看]';    	
			    }else{
    ...
    export class Group extends Contact {
        ...
        constructor(item: {
            ...
            super(item);
            this.unSpeak = item.unSpeak;
            this.imtype = item.imtype;
            this.ida = item.ida;
    ...
    export class ChartRoom extends Contact {
    	unSpeak:string;
    	imtype: string;
    	ida: string;
        upperlimit: number;
